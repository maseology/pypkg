...
class WaterBalanceDataCode:
    ...
    def __init__(self,datatypes=None):
        ...
        if type(datatypes)==DataType:
            ...
            self.dts = [str(dtc.name) for dtc in DataType if int(datatypes) & dtc]
        elif type(datatypes)==int:
            self.wbc = datatypes
            self.dts = [str(dtc.name) for dtc in DataType if datatypes & dtc]
    ...
    def add(self,wbdc):
        if type(wbdc)==DataType:
            self.wbc += int(wbdc)
        elif type(wbdc)==int:
            self.wbc += wbdc
        else:
            print('unknown WaterBalanceDataCode type to add: ' + str(type(wbdc)))
            quit()
        self.__init__(self.wbc)