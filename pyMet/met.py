...
class Met:
    ...
    def __init__(self, fp=None, printout=True, skipdata=True):
        ...
        with open(fp, "rb") as f:
            ...
            # get locations
            if self.lc > 0: rec = self.getLocations(f,rec,printout) 
                ...
                if self.intvl > 0:
                    ...
                    if self.lc < 0:
                        ...
                        while True:
                            ...
                            wbc = wbcode.WaterBalanceDataCode(struct.unpack('<Q', f.read(8))[0]) # waterbudget code
                            nvar = len(wbc.dts)
                            if self.prcn==4:
                                fmt = 'f'*nvar
                                a = np.asarray(struct.unpack('<' + fmt, f.read(4*nvar)))
                            else:
                                fmt = 'd'*nvar
                                a = np.asarray(struct.unpack('<' + fmt, f.read(8*nvar)))
                            coll[dt] = dict(zip(wbc.dts, a))
                        self.dftem = pd.DataFrame.from_dict(coll, orient='index')                    
                        self.wbc = wbcode.WaterBalanceDataCode(list(self.dftem.columns))
                        self.dftem = self.dftem[self.wbc.dts] # re-order columns
                        self.dftem.sort_index(inplace=True)
                    else:
                        print('met.py TODO: irregular data, irregular dates at multiple locations')
                        quit()
    ...
    def getWinters(self, wb=10, we=6):
        ...
        lstt = list()
        blOn = False
        nwin = 0 # number of winters        
        for dt in self.getSortedDates():            
            if blOn:
                if dt.month==we or int((dt-dtl).days) > 64:
                    blOn = False
                    if len(lstt) > 128:
                        lst += lstt
                        nwin += 1
    ...
    def getLocations(self, br, rec, printout):
        if self.lc == 1:
            if self.nloc==1:
